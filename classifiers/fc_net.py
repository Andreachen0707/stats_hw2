...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=1*28*28, hidden_dim=100, num_classes=10, reg=0.0):
        ...
        ############################################################################
        init_weight = 1e-3
        self.params['W1'] = np.random.normal(scale = init_weight,size = (input_dim,hidden_dim))
        self.params['W2'] = np.random.normal(scale = init_weight,size = (hidden_dim,num_classes))
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        out_fcrl_1,cache_fcrl = fc_relu_forward(X,self.params['W1'],self.params['b1'])
        out_fc_2,cache_fc_2 = fc_forward(out_fcrl_1,self.params['W2'],self.params['b2'])
        scores = out_fc_2
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dX_2,grads['W2'],grads['b2'] = fc_backward(grads_out,cache_fc_2)
        dX_1,grads['W1'],grads['b1'] = fc_relu_backward(dX_2,cache_fcrl)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers+1):
            ...
            if i == self.num_layers:
                out_fcrl,cache = fc_forward(out_fcrl,self.params[W_i],self.params[b_i])
            else:
                out_fcrl,cache = fc_relu_forward(out_fcrl,self.params[W_i],self.params[b_i])
        ...
        for i in range(self.num_layers,0,-1):
            ...
            if i ==self.num_layers:
                dx,grads[W_i],grads[b_i] = fc_backward(grads_out,self.cache[cache_i])
            else:
                dx,grads[W_i],grads[b_i] = fc_relu_backward(dx,self.cache[cache_i])
