...
def fc_forward(x, w, b):
    ...
    x_cal = x
    dimension = 1
    for i in range(-1,-1*len(x.shape),-1):
        dimension *=x.shape[i]
...
def fc_backward(dout, cache):
    ...
    for i in range(-1,-1*len(x.shape),-1):
        dimension *=x.shape[i]
    x_cal = x_cal.reshape(x_cal.shape[0],dimension)
    dx = np.dot(dout,w.T)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x,x_norm,running_mean,running_var,gammma,beta,eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x,x_norm,running_mean,running_var,gamma,beta,eps= cache

    N,D = x.shape

    x_mu = x - running_mean
    std_inv = 1./np.sqrt(running_var+eps)
...
def softmax_loss(x, y):
    ...
    #dx = np.dot((score-index).T,x)/x.shape[0]
